...
class BirdTestDataset(Dataset):
    def __init__(self,
        ...
        if single_window:
            self.starts = np.arange(0, 62.5, 2.5)
        else:
            self.starts = np.arange(0, 61.25, 1.25)
...
def compute_pred(LOGITS, thr, incr, weight=0):
    ...
    for file_path, logits in zip(test_audio, LOGITS):
        ...
        for j in range(logits.shape[1]):
            if logits_max[j] > thr:
                logits[:, j] += thr - incr
        for seconds, logit in zip(range(5, 65, 5), logits):
...
df = compute_pred(LOGITS, thr, incr)  # df
df[['row_id', 'birds']].to_csv('res.csv', index=False)
